...
export const BlockScreen: React.FC = () => {
  ...
  return <Indicator show={blockInfoQuery.isLoading}>
    <Container>
      <h2>
        { hash }
      </h2>
      <Properties as="section">
        <PropertiesItem label="Type:">
          ...
          { !blockInfoQuery.data?.confirmed
            ? ' (unconfirmed)'
            : blockInfoQuery.data?.pending ? ' (pending)' : null }
        </PropertiesItem>
        <PropertiesItem label="Account:">
          <Account account={blockInfoQuery.data?.contents.account}/>
        </PropertiesItem>
        { (isSend || isReceive) && <PropertiesItem label="Amount:" big>
        ...
        { isReceive
          ...
          : isSend
            ? <PropertiesItem label="Receiver:">
              <Account account={blockInfoQuery.data?.contents.link_as_account}/>
            </PropertiesItem>
            : null
        }
