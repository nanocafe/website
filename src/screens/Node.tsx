...
export const NodeScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (nodeTelemetryQuery.data?.block_count) {
      ...
      if (lastBlocks.current) {
        setTps((block_count - lastBlocks.current) * 1000 / (timestamp.getTime() - lastTimestamp.current.getTime()) * 1000);
      }
  ...
  return <Indicator show={nodeTelemetryQuery.isLoading || !nodeTelemetryQuery.data}>
    <Container>
      <h2>Node stats:</h2>
      <SplitProperties>
        <Card>
        ...
        <Card>
          ...
          <IntlNumber number={ nodeTelemetryQuery.data?.cemented_count ?? 0 }/>
          </PropertiesItem>
        </Card>
        <Card>
          <PropertiesItem label="Unchecked count:" big>
          <IntlNumber number={ nodeTelemetryQuery.data?.unchecked_count ?? 0 }/>
          </PropertiesItem>
        </Card>
        <Card>
          <PropertiesItem label="Account count:" big>
          <IntlNumber number={ nodeTelemetryQuery.data?.account_count ?? 0 }/>
          </PropertiesItem>
        </Card>
        <Card>
        ...
        <Card>
          <PropertiesItem label="Uptime:" big>
            { dayjs.duration(nodeTelemetryQuery.data?.uptime ?? 0, 'seconds').humanize() }
          </PropertiesItem>
        </Card>
        <Card>
          <PropertiesItem label="Version:" big>
            ...
            { nodeTelemetryQuery.data?.pre_release_version !== '0' && `-rc.${nodeTelemetryQuery.data?.pre_release_version}` }
          </PropertiesItem>
        </Card>
        <Card>
